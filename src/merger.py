...
class AudioMerger:
    ...
    def merge_with_volume(
        ...
        print(f"\n合并音频:")
        print(f"  人声: {vocals_path.name} (音量: {vocals_volume}x)")
        print(f"  背景: {background_path.name} (音量: {background_volume}x)")
    ...
    def _merge_with_python(
        ...
        if max_val > 1.0:
            ...
            print(f"  注意: 音频已归一化（峰值 {max_val:.2f}）")
    ...
    def merge_multiple(
        ...
        print(f"✓ 合并 {len(audio_files)} 个音频: {output_path}")
