...
class M2M100Translator:
    ...
    def __init__(
        ...
        if device == "cuda" and HAS_PERF_CONFIG:
            ...
            if free_mem < required:
                logger.warning(f"显存不足 ({free_mem:.1f}GB < {required}GB)，翻译使用CPU")
                device = "cpu"
    ...
    def load_model(self):
        ...
        try:
            ...
            logger.error("未安装transformers，安装: pip install transformers")
            raise

        logger.info(f"加载M2M100模型: {self.model_name}, 设备: {self.device}")

        if self.model_size == "1.2B":
            logger.info("模型大小约4.5GB，首次下载可能需要10-20分钟...")
        else:
            logger.info("模型大小约1.6GB...")
        ...
        try:
            ...
            self.model.eval()
            logger.info("M2M100模型加载成功，支持100种语言直接互译")
    ...
    def translate_segments(
        ...
        Returns:
            翻译后的片段列表
        """
        logger.info(f"使用M2M100翻译 {len(segments)} 个片段...")
        logger.info(f"{self.source_lang} -> {self.target_lang}")
        ...
        if not valid_texts:
            logger.warning("无有效文本需要翻译")
            return segments

        logger.info(f"有效文本: {len(valid_texts)}/{total}, 批处理大小: {batch_size}")
        ...
        for i in range(example_count):
            idx = valid_indices[i]
            logger.debug(f"原文: {segments[idx].text[:50]}...")
            logger.debug(f"译文: {segments[idx].translated_text[:50]}...")

        translated_count = sum(1 for seg in segments if hasattr(seg, 'translated_text') and seg.translated_text)
        logger.info(f"翻译完成: {translated_count}/{total}")
        return segments
        ...
        for i in range(0, total, batch_size):
            ...
            try:
                ...
                if current % 20 == 0 or current == total:
                    logger.info(f"进度: {current}/{total} ({current * 100 // total}%)")

            except Exception as e:
                logger.error(f"批量翻译失败 [{i}-{i + batch_size}]: {e}")
