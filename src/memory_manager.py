...
class GPUMemoryManager:
    ...
    def get_recommended_device(self, task: str = "general") -> str:
        ...
        if HAS_PSUTIL:
            ...
            if ram_info.percent > 90:
                logger.warning(f"系统内存使用率过高 ({ram_info.percent}%)")
        ...
        if free_mem >= required:
            ...
            if free_mem >= required:
                ...
                logger.warning(f"显存不足 (需要 {required}GB, 可用 {free_mem:.1f}GB)，使用CPU")
                return "cpu"
    ...
    def print_status(self):
        ...
        if self.is_cuda_available:
            logger.info(f"GPU显存: {info.gpu_used:.1f}GB / {info.gpu_total:.1f}GB (空闲: {info.gpu_free:.1f}GB)")
        else:
            logger.info("GPU: 不可用")

        if HAS_PSUTIL:
            logger.info(f"系统内存: {info.ram_used_percent:.1f}% (可用: {info.ram_available:.1f}GB)")
        logger.info("=" * 50)
