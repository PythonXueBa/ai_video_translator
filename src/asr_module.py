...
class WhisperASR:
    ...
    def load_model(self):
        ...
        if self.model is None:
            logger.info(f"加载Whisper模型: {self.model_size} ({self.device})")
    ...
    def transcribe(
        ...
        logger.info(f"识别音频: {audio_path.name}")
    ...
    def transcribe_segments(
        ...
        logger.info(f"并行处理: {max_workers} 个工作进程")
        ...
        for i, segment in enumerate(segments):
            ...
            try:
                ...
                if results:
                    ...
                    logger.debug(f"片段{i}: {segment.text[:50]}...")
                else:
                    segment.text = ""
                    logger.warning(f"片段{i}: 无识别结果")
            except Exception as e:
                logger.error(f"片段{i}: 识别失败 - {e}")
                segment.text = ""
    ...
    def transcribe_with_segments(
        ...
        audio_path = Path(audio_path)
        logger.info(f"识别完整音频: {audio_path.name}")
        ...
        for segment in segments:
            ...
            logger.debug(f"片段{segment.index}: {segment.text[:50]}...")
